...
title Ogrenen Sistem Bakim ve Onarim Araci v1.2.0
color 0A
...
:: LOG (GECMIS SURE) DOSYALARININ YOLU
...
cls
echo =======================================================================
echo          AKILLI SISTEM BAKIM VE ONARIM MENUSU (v1.2.0)
echo =======================================================================
...
:: GECMIS SUREYI OKU VE TAHMIN YAP
...
:: KRONOMETREYI DURDUR VE SANIYE HESAPLA
for /f %%a in ('powershell -nop -c "$end=[long](Get-Date).Ticks; [math]::Round(($end - %START_TICKS%)/10000000)"') do set "SURE_SN=%%a"

if not defined SURE_SN set "SURE_SN=1"
if !SURE_SN! LSS 1 set "SURE_SN=1"
...
:DISK_OPTIMIZE
echo [!] Surucu turu tespit ediliyor (SSD / HDD)...
...
if /i "!DISK_TYPE!"=="SSD" (
    echo [OK] SSD tespit edildi. Sadece TRIM (ReTrim) uygulaniyor (Yazma omru korunur)...
    defrag C: /L /H >nul 2>&1
    echo [OK] SSD TRIM islemi tamamlandi.
) else if /i "!DISK_TYPE!"=="HDD" (
    echo [OK] HDD tespit edildi. Disk birlestirme (Defrag) yapiliyor...
    defrag C: /O /H >nul 2>&1
    echo [OK] HDD birlestirme islemi tamamlandi.
) else (
    echo [UYARI] Disk turu net belirlenemedi. Guvenli TRIM uygulaniyor...
    defrag C: /L /H >nul 2>&1
